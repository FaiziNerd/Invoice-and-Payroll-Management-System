...
export type InvoicePdfOrgContext = {
  companyName?: string;
  companyAddress?: string;
};

function resolveCompany(template?: InvoiceTemplate, org?: InvoicePdfOrgContext) {
  return {
    name:
      org?.companyName?.trim() ||
      template?.branding.companyName ||
      DEFAULT_COMPANY_PLACEHOLDER,
    address: org?.companyAddress?.trim() || template?.branding.companyAddress || "",
...
function ClassicPDF({ invoice, client, template, org }: InvoicePDFProps) {
  const { name, address, primaryColor, branding } = resolveCompany(template, org);
...
function ModernPDF({ invoice, client, template, org }: InvoicePDFProps) {
  const { name, address, primaryColor, branding } = resolveCompany(template, org);
  const styles = baseStyles(primaryColor);
...
function MinimalPDF({ invoice, client, template, org }: InvoicePDFProps) {
  const { name, address, primaryColor, branding } = resolveCompany(template, org);
  const styles = baseStyles(primaryColor);
    ...
    <Page size="A4" style={[styles.page, { paddingTop: 50 }]}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 30 }}>
        <View>
          ...
          <Text style={{ fontSize: 22, fontWeight: "normal", marginTop: 4 }}>
      ...
      >
        <View>
          <Text style={{ fontSize: 8, letterSpacing: 2, color: "#999", marginBottom: 6 }}>
            BILL TO
          </Text>
          <Text style={{ fontWeight: "bold" }}>{client.name}</Text>
          <Text>{client.email}</Text>
...
export function InvoicePDFDocument({ invoice, client, template, org }: InvoicePDFProps) {
  const theme = template?.theme || "classic";

  return (
    <Document>
      {theme === "modern" ? (
        <ModernPDF invoice={invoice} client={client} template={template} org={org} />
      ) : theme === "minimal" ? (
        <MinimalPDF invoice={invoice} client={client} template={template} org={org} />
      ) : (
        <ClassicPDF invoice={invoice} client={client} template={template} org={org} />
      )}
    </Document>
  );
}
